...
class BrandAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'slug', 'description_preview', 'logo_preview', 'logo_size_display', 'seo_title_display', 'is_active', 'created_by_display', 'updated_by_display', 'created_at_thai')
    list_filter = ('is_active', 'created_at', 'updated_at')
    search_fields = ('name', 'slug', 'description', 'seo_title', 'seo_description')
    readonly_fields = ('created_at_thai', 'updated_at_thai', 'created_by_display', 'updated_by_display', 'logo_preview', 'logo_size_display', 'og_image_preview', 'og_image_size_display', 'logo_with_delete_button', 'og_image_with_delete_button')
    ordering = ('name',)
    prepopulated_fields = {'slug': ('name',)}
    actions = ['delete_brand_logos']
    ...
    fieldsets = (
        ...
        ('Logo แบรนด์', {
            'fields': ('logo_with_delete_button', 'logo', 'logo_size_display', 'alt_text')
        }),
        ('SEO Optimization', {
            'fields': ('seo_title', 'seo_description'),
            'classes': ('collapse',),
    ...
    def logo_preview(self, obj):
        ...
        if obj.logo:
            return mark_safe(
                f'<div class="brand-logo-preview"><img src="{obj.logo.url}" alt="Brand Logo" /></div>'
            )
        return "ไม่มีรูป"
    logo_preview.short_description = 'Logo แบรนด์'
    logo_preview.allow_tags = True
    
    def logo_size_display(self, obj):
        """Display logo size"""
        return obj.logo_size
    logo_size_display.short_description = 'ขนาด Logo'
    ...
    def logo_with_delete_button(self, obj):
        """Display logo preview with delete button for detail view"""
        if obj.logo:
            delete_url = f'/admin/products/brand/{obj.pk}/delete-logo/'
            return mark_safe(
                f'''
                <div class="brand-logo-preview">
                    <img src="{obj.logo.url}" alt="Brand Logo" />
                    <br>
                    <a href="{delete_url}" class="button" style="margin-top: 10px; background: #dc3545; color: white; padding: 5px 10px; text-decoration: none; border-radius: 3px;" 
                       onclick="return confirm('คุณแน่ใจหรือไม่ที่จะลบโลโก้แบรนด์นี้?')">
                        ลบโลโก้
                    </a>
                </div>
                '''
        ...
        return "ไม่มีรูป"
    logo_with_delete_button.short_description = 'Logo แบรนด์'
    logo_with_delete_button.allow_tags = True
    
    def og_image_with_delete_button(self, obj):
        """Display OG image preview with delete button for detail view"""
        if obj.og_image:
            delete_url = f'/admin/products/brand/{obj.pk}/delete-og-image/'
            return mark_safe(
                f'''
                <div class="og-image-preview">
                    <img src="{obj.og_image.url}" alt="OG Image" />
    ...
    def delete_brand_logos(self, request, queryset):
        """Delete logos for selected brands"""
        deleted_count = 0
        for brand in queryset:
            try:
                # Delete main logo
                if brand.logo:
                    if default_storage.exists(brand.logo.name):
                        default_storage.delete(brand.logo.name)
                    brand.logo = None
                
                # Delete OG image
                if brand.og_image:
                    if default_storage.exists(brand.og_image.name):
                        default_storage.delete(brand.og_image.name)
                    brand.og_image = None
                
                brand.save()
                deleted_count += 1
            except Exception as e:
                messages.error(request, f"Error deleting logos for {brand.name}: {e}")
        
        if deleted_count > 0:
            messages.success(request, f"ลบโลโก้แบรนด์ {deleted_count} รายการเรียบร้อยแล้ว")
        else:
            messages.warning(request, "ไม่มีโลโก้แบรนด์ที่สามารถลบได้")
    
    delete_brand_logos.short_description = "ลบโลโก้แบรนด์"
    
    def get_urls(self):
        """Add custom URLs for logo management"""
        urls = super().get_urls()
        custom_urls = [
            path('<int:object_id>/delete-logo/', self.delete_logo_view, name='products_brand_delete_logo'),
            path('<int:object_id>/delete-og-image/', self.delete_og_image_view, name='products_brand_delete_og_image'),
        ]
        return custom_urls + urls
    
    def delete_logo_view(self, request, object_id):
        """Delete main logo for specific brand"""
        ...
        if brand.logo:
            try:
                ...
                brand.logo = None
                brand.save()
                messages.success(request, f"ลบโลโก้แบรนด์ {brand.name} เรียบร้อยแล้ว")
            except Exception as e:
                messages.error(request, f"Error deleting logo: {e}")
        else:
            messages.warning(request, "ไม่มีโลโก้แบรนด์ที่สามารถลบได้")
        
        return redirect(f'/admin/products/brand/{object_id}/change/')
    
    def delete_og_image_view(self, request, object_id):
        """Delete OG image for specific brand"""
        brand = get_object_or_404(Brand, pk=object_id)
        ...
        if brand.og_image:
            try:
                ...
                brand.og_image = None
                brand.save()
                messages.success(request, f"ลบรูป OG ของแบรนด์ {brand.name} เรียบร้อยแล้ว")
            except Exception as e:
                messages.error(request, f"Error deleting OG image: {e}")
        else:
            messages.warning(request, "ไม่มีรูป OG ที่สามารถลบได้")
